...
async def admin_delete_user(
    user_id: PydanticObjectId, current_user: TokenData = Depends(get_user)
):
    # Check if user is admin
    admin_user = await User.find_one(User.username == current_user.username)
    if not admin_user or not admin_user.is_admin:
...
async def upload_file(
    file: UploadFile = File(...), user: TokenData = Depends(get_user)
):
    ...
    existing_user.documents.append({"name": file.filename, "url": file_url})
